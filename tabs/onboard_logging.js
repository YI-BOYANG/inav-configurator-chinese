...
TABS.onboard_logging.initialize = function(callback) {
    ...
    function populateDevices() {
        ...
        deviceSelect.append('<option value="0">串行端口</option>');
        if (DATAFLASH.ready) {
            deviceSelect.append('<option value="1">板载闪存芯片</option>');
        }
        if (SDCARD.supported) {
            deviceSelect.append('<option value="2">板载SD卡槽</option>');
        }
    ...
    function update_html() {
        ...
        update_bar_width($(".tab-onboard_logging .sdcard-other"), SDCARD.totalSizeKB - SDCARD.freeSizeKB, SDCARD.totalSizeKB, "不可用空间", true);
        update_bar_width($(".tab-onboard_logging .sdcard-free"), SDCARD.freeSizeKB, SDCARD.totalSizeKB, "日志剩余空间", true);
